  ...
  <v-container grid-list-xl>
    ...
    >
      ...
      >
        <v-card
          class="my-5"
          v-for="(session, index) in courseContent"
          :key="index"
        >
          <v-card-media
            :src=session.image
          ...
          <v-card-title primary-title>
            <div>
              ...
              <v-btn
                icon
                @click.native="visible.includes(index)? visible.splice(visible.indexOf(index), 1): visible.push(index)"
              >
      ...
      >
        ...
        <blockquote class="pl-3 pb-3 text-xs-justify">
          <GmapMap
            :center="{lat: 19.0369, lng: 72.8584}"
            :zoom="16"
            map-type-id="hybrid"
            :style=getDimensions()
          >
          ...
          <br> 143 / B Padmalaya, 1st floor, Sion Jain Society. Mumbai 400022
        </blockquote>
        <h2 class="headline blue--text pl-3 pb-2 ">Pricing</h2>
        <blockquote class="pl-3 pb-3 text-xs-justify">The course will be broken down into 11 - 12 sessions of 3 hours each. Hands on coding practice is ensured which is why machines are required. We need a machine for each session. We need you to understand that is not a theory course. We get our hands dirty and learn to code. Also, we will have a separate session - 12th session to understand softskills required for many dream company interviews</blockquote>
        <v-list
          three-line
          subheader
        >
      ...
      pricing: [
        {
          cost: "5000",
          details:
            "You most defnitely need a laptop for this course. We will experience coding in all its glory."
        }
      ],
      courseContent: [],
      visible: [],
      markers: [{ position: { lat: 19.0369, lng: 72.8584 } }]
    };
  },
  methods: {
    onResize() {
